...
void setup() {
  ...
  pinMode(pulse_sellect, OUTPUT);
  lcd.begin(LCD_COLUMNS, LCD_ROWS);
  lcd.clear();
  lcd.setCursor(0, 0);
  lcd.print("DYNLAB-----V1.01");
  lcd.setCursor(0, 1);
  lcd.print("Peristaltic-Pump");
  delay(1800);
  lcd.clear();
  lcd.setCursor(0, 0);
  lcd.print("Detecting Link");
  delay(500);
  lcd.setCursor(14, 0);
  lcd.print(".");
  delay(500);
  ...
  if (digitalRead(link) == 1) {
    digitalWrite(pulse_sellect, HIGH);
    lcd.setCursor(0, 1);
    lcd.print("Host mode");
  }
  if (digitalRead(link) == 0) {
    digitalWrite(pulse_sellect, LOW);
    lcd.setCursor(0, 1);
